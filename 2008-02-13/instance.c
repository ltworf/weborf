...
extern pthread_mutex_t m_free;//Mutex per modificare quel valore
...
void * instance(void * nulla){
	...
	signal(SIGPIPE, piperr);//Handles SIGPIPE
...
int request_auth(int sock,char * descr) {
	...
	sprintf(page,"%s<H1>Authorization required</H1><p>%s</p>%s",HTMLHEAD,descr,HTMLFOOT);
	
	//Preparo l'header http	
	int page_len = strlen(page);

	sprintf(head,"HTTP/1.1 401 Authorization Required: Weborf (GNU/Linux)\r\nContent-Length: %d\r\nWWW-Authenticate: Basic realm=\"%s\"\r\n\r\n",page_len,descr);
	...
	printf("Libero memoria pagina..\n");
	
	free(page);
	
	printf("Libero memoria head..\n");
