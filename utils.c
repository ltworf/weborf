...
*/
int
list_dir (char *dir, char *html, unsigned int bufsize, bool parent)
{
  ...
  if (dp != NULL)
    {				//Open succesfull
      ...
      strncat (html, "<table><tr><td></td><td>Name</td><td>Size</td></tr>",
	       maxsize);
	...
	{			//Cycles trough dir's elements
	  ...
	  if (S_ISREG (f_mode))
		...
		{		//Parent entry
	      ...
	      else if (ep->d_name[0] == '.' && ep->d_name[1] == '\0')
		{		//Self entry
		  //doesn't show it
		  print = false;
		}
...
get_param_value (char *http_param, char *parameter, char *buf, int size)
{
  ...
  if (val == NULL)
	...
	{			//Parameter string is too long for the buffer
	  return false;
	}
...
setIpEnv ()
{
  ...
  if (e_host == NULL)
    {				//If no host is supplied in the request
      return -1;
    }
