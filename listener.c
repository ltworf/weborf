...
*/
void
init_threads (unsigned int count)
{
...
main (int argc, char *argv[])
{
  ...
  while (1)
	...
	{0, 0, 0, 0}
      ...
      c =
	getopt_long (argc, argv, "mvhp:i:I:u:dxb:a:V:", long_options,
		     &option_index);
  ...
  if (ip == NULL)
    {				//Default ip, listens to all the interfaces
  ...
  if (inet_aton (ip, &locAddr.sin_addr) == 0)
  ...
  while ((s1 =
		  ...
		  (socklen_t *) & farAddrL)) != -1)
    {
      ...
      if (s1 >= 0 && t_free > 0)
	{			//Adds s1 to the queue
	  q_put (&queue, s1, farAddr);
	}
      else
	{			//Closes the socket if there aren't enough free threads.
#ifdef REQUESTDBG
	...
	{			//Need to start new thread
	  if (thread_c + INITIALTHREAD < MAXTHREAD)
	    {			//Starts a group of threads
	      init_threads (INITIALTHREAD);
	    }
	  else
	    {			//Can't start a group because the limit is close, starting less than a whole group
...
setAuthbin (char *bin)
{
  ...
  if (system (command) != 0)
    {				//Doesn't exist or it isn't executable
  ...
  for (;;)
    {
      ...
      if (t_free > MAXFREETHREAD)
	{			//Too much free threads, terminates one of them
