...
void modURL(char* url) {
	...
	{//Rimpiazzare le sequenze di escape
		char e_seq[3];
		e_seq[2]=0;
		
		int i;
		int url_len=strlen(url);//Lunghezza url
		for (i=0;i<url_len-2;i++) {
			if (url[i]=='%') {
				
				//Mette nel buffer l'escape
				e_seq[0]=url[i+1];
				e_seq[1]=url[i+2];
						
				delChar(url,i,2);//Elimina 2 caratteri
				
				//Sostituisce il 3o con il carattere dell'escape
